...
import {favoriteTranslations} from './favorite.translations';
...
export class FavoriteComponent implements OnInit, OnDestroy {
  ...
  protected translations: FavoriteTranslationType;

  serverImagesPath: string = environment.images;
  favoriteProducts: FavoriteProductType[] = [];
  cartProducts: CartItemType[] | null = null;

  constructor() {
    this.appLanguage = this.languageService.appLang;
    this.translations = favoriteTranslations[this.appLanguage]
  }
  ...
  private applyCartChanges(favoriteProducts: FavoriteProductType[] | null = null): void {
    let favProducts: FavoriteProductType[] = favoriteProducts ? favoriteProducts : this.favoriteProducts;
    if (this.cartProducts) {
      favProducts.forEach((favProduct: FavoriteProductType) => {
    ...
    if (favoriteProducts) this.favoriteProducts = favProducts;
  }
    ...
    this.subscriptions$.add(
      combineRequests$.subscribe({
        next: ([cartResp, favoritesResp]: [CartResponseType, FavoritesResponseType]) => {
          ...
          this.applyCartChanges(favoritesResp.favorites);
        },
  ...
  private doFavRequest(): void {
    this.subscriptions$.add(this.favoriteService.getFavorites().subscribe({
      next: (data: FavoritesResponseType) => {
        ...
        this.applyCartChanges(data.favorites);
      },
      error: (errorResponse: HttpErrorResponse) => {
  ...
  ngOnInit() {
    this.subscriptions$.add(this.languageService.currentLanguage$.subscribe((language: AppLanguages) => {
      ...
      //Если this.cartProducts не null, то происходит смена языка и основной запрос уже был, поэтому корзину не запрашиваем
      this.cartProducts ? this.doFavRequest() : this.doRequests();
    }));
  }
