export class ResponseDataValidator {
   ...
   * Если поле не обязательно - в шаблоне помечаем как undefined
   * Если может прити пустая строка или null - помечаем как null
  ...
  static validateRequiredFields<T extends Record<string, any>>(template: T, data: any): boolean {
    ...
    for (const key of Object.keys(template)) {
      ...
      const isOptional = expected === undefined;

      // Если поле опциональное и отсутствует в данных — пропускаем
      if (isOptional && !(key in data)) continue;

      // Обязательное поле отсутствует
      if (!isOptional && !(key in data)) return false;

      // Проверяем пустые значения только для обязательных или присутствующих опциональных
      if (!this.isValueAllowedNull(expected) && this.isEmpty(value)) return false;
  ...
  private static isEmpty(value: any): boolean {
    ...
    if (value === null) return false; // null допустим
  ...
  private static isValueAllowedNull(templateValue: any): boolean {
